-- ============================================================
-- 006 — PUESTA A PUNTO (auditoría 2026-07-05)
-- Ejecutar en Supabase → SQL Editor. Idempotente: se puede
-- correr más de una vez sin romper nada.
-- Incluye:
--   1) Bucket de imágenes 'producto-imagenes' + políticas
--   2) Solicitudes de pedido (tabla + RPC) — contenido de 005
--   3) Limpieza de tablas viejas en inglés (solo si están vacías)
-- ============================================================

-- ──────────────────────────────────────────────
-- 1) BUCKET DE IMÁGENES + POLÍTICAS DE STORAGE
-- ──────────────────────────────────────────────
...
DROP POLICY IF EXISTS "imagenes_actualizar_staff" ON storage.objects;
CREATE POLICY "imagenes_actualizar_staff"
...
-- ──────────────────────────────────────────────
-- 2) SOLICITUDES DE PEDIDO (tabla + items + RPC)
-- ──────────────────────────────────────────────
...
--    (en el título, "|" separa la parte resaltada en naranja)
-- ──────────────────────────────────────────────
UPDATE site_content SET valor = 'Todo lo que necesitás para tu hogar,|en un solo lugar.'
 WHERE clave = 'hero_titulo' AND valor = 'Todo para tu hogar, en un solo lugar.';

UPDATE site_content SET valor = 'Encontrá electrodomésticos, muebles, herramientas y tecnología con precios mayoristas y minoristas. Comprá online o recibí atención personalizada por WhatsApp.'
 WHERE clave = 'hero_subtitulo' AND valor LIKE 'Electrodomésticos, muebles, herramientas%';

UPDATE site_content SET etiqueta = 'Título principal (usá | antes de la parte naranja)'
